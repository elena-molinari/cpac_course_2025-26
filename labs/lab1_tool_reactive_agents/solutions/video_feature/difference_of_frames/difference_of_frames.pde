...
void effectDiffFrames(PImage img){
  ...
  for(int loc=0; loc<cam.width*cam.height; loc++){
    ...
    color c1 = cur_frame.pixels[loc];
    color c2 = old_frame.pixels[loc];
    
    float rdiff = abs(red(c1) - red(c2));
    float gdiff = abs(green(c1) - green(c2));
    float bdiff = abs(blue(c1) - blue(c2));
    
    normdiff = map((rdiff + gdiff + bdiff) / 3.0, 0, 255, 0, 1); 
