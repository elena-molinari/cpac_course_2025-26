...
void setup() {
  ...
  for (int i=0; i<N_AGENTS; i++) {
    movers[i] = new AgentMover(mass_mover, new SoundFile(this, filenames[i]));
  }
...
float computeDistance(AgentMover mover) {
  PVector dist = mover.position.copy();
  dist.sub(pos_attractor);
...
void draw() {
  PVector force;
  /* YOUR CODE HERE: modify this function to draw multiple movers (step 1) */
  for (int i = 0; i< N_AGENTS; i++) {
    force = computeGravityForce(movers[i]);
    dist = computeDistance(movers[i]);
    movers[i].applyForce(force);
    movers[i].update();
    movers[i].changeAmp(dist);
    movers[i].draw();
  }
  rectMode(CORNER);
  fill(0, 20);
